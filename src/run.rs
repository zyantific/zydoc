...
struct Args {
    ...
    #[argh(option)]
    repo: path::PathBuf,
    /// git references to generate documentation for (regex)
    #[argh(option)]
    refs: Vec<String>,
    /// git references to exclude (regex)
    #[argh(option)]
    ...
    /// extra CSS files to use for Doxygen builds
    #[argh(option)]
    extra_css: Vec<path::PathBuf>,
}
...
pub fn run() -> Result<()> {
    ...
    let regexps = compile_regexps(&args.refs)?;
    let exclude_regexps = compile_regexps(&args.exclude_refs)?;
    ...
    for git_ref in repo.refs()?.into_iter().rev() {
        ...
        if exclude_regexps.iter().any(|re| re.is_match(&git_ref)) {
            continue;
        }
        ...
        let extra_css = args
            .extra_css
            .iter()
            .map(|x| x.canonicalize().unwrap().to_string_lossy().into_owned())
            .collect::<Vec<_>>()
            .join(" ");
...
fn compile_regexps(regexps: &[String]) -> Result<Vec<regex::Regex>> {
    regexps
        .iter()
